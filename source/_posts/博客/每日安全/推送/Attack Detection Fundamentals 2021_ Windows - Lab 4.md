
---
title: 'Attack Detection Fundamentals 2021_ Windows - Lab #4'
categories: 
 - 博客
 - 每日安全
 - 推送
headimg: 'https://picsum.photos/400/300?random=1237'
author: 每日安全
comments: false
date: 2021-04-09 00:26:43
thumbnail: 'https://picsum.photos/400/300?random=1237'
---

<div>   
<q>Attack Detection Fundamentals 2021 Windows Lab #4：窃取目标用户的 Chrome 浏览器中的 cookie，并探索如何检测这类可疑的劫持行为。</q>
–
<cite>
<a class="text-muted" href="https://sec.today/user/dc071663-2f31-4d9f-928e-150b2cafeef3/pushes/">
potato
</a>
<span class="text-muted"><small>• 19 hours ago</small></span>
</cite>
  
</div>
            