...
<div>   
<br>K8s的控制器和Worker节点都包含多个组件。如果你是构建了一个集群的K8s管理员，并且用户告诉你他们已经可以在集群上成功部署和测试应用程序了。<br>
然而，集群是否能一直保持健康状态不出现什么问题，这一点却没什么保证。任何时候都有可能出问题，服务可能停止运行，K8s组建也可能工作不正常等等。因此，很有必要为K8s集群构建可靠的监控方案。<br>
<br>本文尝试解决：<br>
...
<br>
<br><h3>为什么需要监控</h3>K8s集群包括多个组件和层次，都有不同的故障点。知道什么时候以及为什么发生故障至关重要。监控系统的目标就是辅助提供健康可靠的系统。<br>
除了提供可靠性，监控系统也可以帮助大家理解系统，有助于分析各种问题，比如故障，资源使用情况等等。<br>
另一个使用监控的好处是可以理解并发现系统的趋势来辅助资源规划。<br>
一些用户场景包括（不限于）：<br>
<blockquote><br>监控K8s集群和节点</blockquote>集群资源的使用，集群/节点可用性，健康情况等。<br>
<blockquote><br>监控K8s部署，服务和pod</blockquote>失败的pod/部署/服务期待一些运行着的副本，pod运行有资源的请求和限制，等<br>
<blockquote><br>监控K8s应用程序</blockquote>可用性，健康状况，性能等<br>
...
<br>
<br><h3>监控指标</h3>在不同的监控层面可以收集不同的监控指标。下图展示了在不同的层面可以收集哪些指标。<br>
<br><div class="aw-upload-img-list active">
...
<br>
<br><h3>监控工具</h3>> K8s有什么内建的监控工具吗？<br>
<br>K8s提供了内建的监控工具。只需很少的配置就可以开箱即用，不过，也有其他更多更强大的工具。<br>
K8s集群里的每个节点上都运行着kubelet组件。每个kubelet包含<code class="prettyprint">cAdvisor</code>，它收集所在节点的每个容器的CPU，内存这些指标。它也帮助收集节点上的其他指标。<br>
<br><code class="prettyprint">Metrics Server</code>从cAdvisor收集指标并归总给中央控制平台。Metrics Server是在K8s集群的一些节点上运行着的pod。当这些指标收集上来后，用户可以运行<code class="prettyprint">kubectl top</code>查看容器，pod或节点的CPU，内存和网络使用情况。<br>
<br><code class="prettyprint">K8s Dashboard</code>提供了Metrics Server的数据的可视化展示。<br>
<br><code class="prettyprint">K8s State Metrics</code>帮助提供Metrics Server无法提供的额外的指标。它监听K8s API，生成和K8s逻辑对象相关的指标，比如，节点状态，节点容量，pod状态等。它可以部署为单个副本的服务。<br>
<br><code class="prettyprint">Probe</code>帮助监控容器和服务的健康状态。<code class="prettyprint">Liveness</code>探针帮助检查服务/pod是否还在运行，如果没有运行就采取措施。<code class="prettyprint">Readiness</code>探针帮助检测服务/pod是否能够接受访问流量。<a href="https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/">K8s官方文档</a>详细介绍了如何配置这些探针。<br>
<br><div class="aw-upload-img-list active">
...
<br>
<br>推荐在生产环境上使用更成熟的开源或商业化的监控解决方案。下一篇博文会介绍其他监控方案，监控流水线和监控架构。<br>
