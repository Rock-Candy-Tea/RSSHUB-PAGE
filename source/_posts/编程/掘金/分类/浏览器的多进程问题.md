...
<h4 data-id="heading-1">1.browser进程</h4>
<pre><code class="copyable">1.浏览器的主进程，只有一个
2.负责浏览器的界面显示、与用户交互，如前进后退
3.负责其他页面的管理，创建、销毁其他进程
4.将renderer进程得到的内存中bitmap，绘制到用户界面上
<span class="copy-code-btn">复制代码</span></code></pre>
<h4 data-id="heading-2">2.unitify Network进程</h4>
<pre><code class="copyable">1.负责网络请求、下载等
<span class="copy-code-btn">复制代码</span></code></pre>
<h4 data-id="heading-3">3.插件进程</h4>
<pre><code class="copyable">1.每种类型的插件对应一个进程，且仅当使用时才创建
<span class="copy-code-btn">复制代码</span></code></pre>
<h4 data-id="heading-4">4.GPU进程</h4>
<pre><code class="copyable">1.最多一个，用于3D渲染
    
<span class="copy-code-btn">复制代码</span></code></pre>
<h4 data-id="heading-5">5.render进程</h4>
<pre><code class="copyable">1.用于执行页面渲染、脚本执行、事件处理等
...
<span class="copy-code-btn">复制代码</span></code></pre>
<p><img src="https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/6bd23c7520e14570af5d73b2883f000e~tplv-k3u1fbpfcp-watermark.image" alt="image.png" loading="lazy" referrerpolicy="no-referrer"></p>
<pre><code class="copyable">9.webworker
    html5新增，通过possMessage通讯
    具体另外补充
    
<span class="copy-code-btn">复制代码</span></code></pre>
<h4 data-id="heading-6">6.浏览器渲染过程</h4>
<pre><code class="copyable">1.浏览器输入url后经历了什么
...
b.会阻塞render树渲染
<span class="copy-code-btn">复制代码</span></code></pre>
<h4 data-id="heading-7">7.浏览器渲染步骤</h4>
<pre><code class="copyable">a.解析html构建dom树
...
<span class="copy-code-btn">复制代码</span></code></pre>
<p><img src="https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/d691040bc3a9425eb79a96434121593b~tplv-k3u1fbpfcp-watermark.image" alt="image.png" loading="lazy" referrerpolicy="no-referrer"></p>
<h4 data-id="heading-8">8.js运行机制</h4>
<pre><code class="copyable">1.js为单线程
    // 这里需要做补充，暂时不做
2.js任务分为同步任务和异步任务
...
5.js运行过程拓展
    ...
    3).声明方式的函数作用域为调用他外层函数fn2内或者LE中（window）
    4).let、const、var区别，备注一下，下次专门补充
