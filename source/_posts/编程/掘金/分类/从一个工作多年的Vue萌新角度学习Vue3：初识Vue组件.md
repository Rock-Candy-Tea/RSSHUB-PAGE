...
---
title: '从一个工作多年的Vue萌新角度学习Vue3：初识Vue组件'
categories: 
 - 编程
 - 掘金
 - 分类
...
<h1 data-id="heading-0">先跑起来再说</h1>
<p>对于一个小白来说，要学习一门新技术，最快的方式就是：</p>
<blockquote>
<p>先跑起来再说</p>
</blockquote>
<p>跑起来之后，我们会对这门新技术有一个感觉的印象，之后看文档也会更清晰。</p>
...
<p>Vite除了创建Vue的工程，还可以常见React/Preact/Svelte等多种框架的工程。</p>
...
<h1 data-id="heading-1">Vue组件初步印象</h1>
...
<p>这个文件是以.vue为文件后缀的，代表这是一个<code>Vue组件</code>。</p>
<p>一个Vue组件包含三个部分：</p>
<ul>
...
<h1 data-id="heading-2"><template>分析</h1>
...
<p>我们尝试修改下template里面的内容，比如将最后一行的：</p>
...
<p>从以上动图可以看出，修改完<code>template</code>中的内容一保存文件，页面内容立马刷新，几乎没有任何延迟，页面也没有刷新，开发体验非常丝滑。</p>
...
<h1 data-id="heading-4"><script>分析</h1>
...
<h2 data-id="heading-5">导入Vue方法</h2>
...
<h2 data-id="heading-6">导出Vue组件</h2>
<p>第39行导出了一个Vue组件。</p>
...
<p>从哪儿传进来的？使用组件是通过它的名字<code>name</code>来使用的，所以我们在源代码里面搜索组件的名字：<code>HelloWorld</code>，发现是在<code>App.vue</code>中使用的：</p>
...
<span class="copy-code-btn">复制代码</span></code></pre>
<h2 data-id="heading-8">使用Vue组件</h2>
...
<h2 data-id="heading-9">Vite热更新 - script</h2>
<p>这时我们尝试修改下这个msg的值（比如改成：<code>Hello everyone! I'm learning Vue 3 + TypeScript + Vite</code>），看下页面会有什么变化。</p>
...
<p>从以上动图可以看出，与修改<code>template</code>的效果一样，修改完msg的值一保存文件，页面内容立马刷新，之前的：</p>
...
<h2 data-id="heading-10">响应式的ref对象</h2>
...
<pre><code class="copyable">  setup: () => &#123;
  ...
  &#125;
<span class="copy-code-btn">复制代码</span></code></pre>
<p>之前我们在<code>template</code>中也见过这个变量，它的值就是这里定义的<code>count</code>，我们注意到这个<code>count</code>的值调用一个叫<code>ref</code>的函数之后返回的，函数的参数是一个数字<code>0</code>。为什么要包一层ref，而不是直接将0赋值给count变量呢？</p>
...
<span class="copy-code-btn">复制代码</span></code></pre>
<p>为了理解ref函数的作用，我们先尝试在页面里面点击一下这个<code>count is: 0</code>的button按钮。</p>
...
<pre><code class="hljs language-js copyable" lang="js"><span class="hljs-keyword">const</span> count = <span class="hljs-number">0</span>
<span class="copy-code-btn">复制代码</span></code></pre>
<p>这时再次点击button按钮，发现值没有变。</p>
<p>这时我们大致能理解ref函数返回<code>响应式ref对象</code>的含义：</p>
<blockquote>
<p>响应式的意思就是这个变量的值是动态的，某些动作（点击按钮）改变了它的值，模板里面的文本插值立马也会跟着变化，从而页面里面的内容也会跟着刷新。</p>
</blockquote>
<p>如果没有count被ref函数包裹，那它就不是响应式的，按钮按钮改变它的值之后，模板的内容不会跟着变化。</p>
<p>有一个需要注意的点：</p>
<blockquote>
<p>setup中定义的变量必须返回，才能在template中使用，否则插值不会被渲染，并且会在浏览器控制台警告提示</p>
</blockquote>
...
<span class="copy-code-btn">复制代码</span></code></pre>
<h2 data-id="heading-11">TypeScript支持</h2>
...
<span class="copy-code-btn">复制代码</span></code></pre>
<h2 data-id="heading-12">TypeScript类型错误高亮提示</h2>
...
<span class="copy-code-btn">复制代码</span></code></pre>
<p>如果将size复赋值为Size类型为定义的值，比如：<code>large</code>，<a href="https://link.juejin.cn/?target=https%3A%2F%2Fvuejs.github.io%2Fvetur%2F" target="_blank" rel="nofollow noopener noreferrer" title="https://vuejs.github.io/vetur/" ref="nofollow noopener noreferrer">Vetur</a>类型检查马上就会提示，相应的赋值代码也会出现红色波浪下划线：</p>
...
<li>使用Vue组件和使用html元素差不多，就是需要先导入、声明组件才能使用</li>
<li>ref用于返回一个响应式对象</li>
<li><code>lang="ts"</code>用来支持TypeScript</li>
</ol>
<h1 data-id="heading-14"><style>分析</h1>
...
<span class="copy-code-btn">复制代码</span></code></pre>
<p>看着和写CSS没什么区别，只是有一点不一样（前面也提到过），就是<code><style></code>标签中增加了一个<code>scoped</code>属性，这个属性用来定义局部组件样式，里面写的样式只针对当前组件生效。</p>
<h2 data-id="heading-15">布局组件样式</h2>
...
<p>我们发现在HelloWorld组件的style中写的样式并不会影响App组件中的code，这就是局部样式。</p>
<p>通过对比两者的html元素，我们发现HelloWorld组件中的元素都加上了一个<code>data-v-</code>开头的特殊属性，相应的css规则也加上了这个选择器。</p>
...
<p>这一点和Angular的<code>encapsulation</code>属性的作用非常类似。</p>
<h2 data-id="heading-16">Vite热更新 - style</h2>
<p>除了<code>template</code>和<code>script</code>的热更新，Vite也支持<code>style</code>样式的热更新，一样的丝滑，就不再赘述。</p>
<h1 data-id="heading-17">小结</h1>
<p>通过本文，我们使用Vite启动了一个初始的项目工程，并且对Vue组件有了一个初步的认识，现在做一个简单的小结巩固下吧。</p>
<ul>
