...
<pre><code class="copyable">JavaScript 代码必须位于<script></script>标签内。可放置与HML中的head、body中
属性有：src(外部引入)、type
...
重复声明同一变量，如果不赋值，返回不赋值往前的有赋值的值；如果赋值，将会是最后一个的值。</p>
...
<code>+</code> 运算符可用于字符串相加（concat）;字符串与数字相加返回字符串（concat）。
<img src="https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/469ba5f113154794b8d460b99f37a935~tplv-k3u1fbpfcp-watermark.image" alt="image.png" loading="lazy" referrerpolicy="no-referrer"></p>
