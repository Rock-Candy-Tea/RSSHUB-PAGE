...
---
title: 'itest work 接口测试&敏捷测试管理平台 9.5.0 GA_u3，优化及 bug 修复'
categories: 
 - 编程
 - 开源中国
 - 资讯
...
author: 开源中国
comments: false
date: Fri, 16 Apr 2021 09:51:00 GMT
...
<div class="content">
 ...
 <div> 
  <div> 
   ...
   <p><strong>v9.5.0  GA _u3下载地址 </strong>：<a href="https://www.oschina.net/action/GoToLink?url=http%3A%2F%2F120.78.0.137%2Frsf%2Fsite%2Fitest%2Fdownload%2Findex.html" target="_blank">itest下载</a></p> 
   <p><strong>二：9.5.0 GA_u3更新说明</strong></p> 
   <p>9.5.0 <strong>GA</strong>_u1<strong> </strong> 4月9号发布后，带着14个更新<strong>(不含 GA_u2的9个更新)</strong>且含1个BUG修复的9.5.0  GA_u3 又来了<strong>，用户的持续反馈是我们不断更新的动力!4月底发布新一版源码到开源中国上，当前忙着快速迭代，接口测试和实现压测功能。另外在此也预告一下大版本压测功能4月22发布。</strong></p> 
   ...
   <div>
    <strong><strong>9.5.0 GA_u3 详情如下:</strong></strong>
   </div> 
   <div>
    <strong>13个增强：</strong>
   </div> 
   <div>
    <strong>1、</strong>产品用例库，修改和查看详情合到一个连接中。
    <br> 
    <strong>2、</strong>产品用例库 ，实现脑图显示模式。
    <br> 
    <strong>3、</strong>公共用例库 ，实现脑图显示模式。
    <br> 
    <strong>4、</strong>用户列表中，是否为管理员改为查看所有项目,撤销查看所有项目，设置查看所有项目。
    <br> 
    <strong>5、</strong>公共用例库，以前是按身份类型来决定有没有审权权，改为权限设置来，和身份无关。
    <br> 
    <strong>6、</strong>执行用例时，检查预期结果和步骤以及描述有没有改，要是做了修改，自动保存并执行。
    <br> 
    <strong>7、</strong>mock head 参数加下拉列表，方便设置常用的head 头。
    <br> 
    <strong>8、</strong>优化当项目有50多个时，总揽中统计未分配用例，显示明显有卡顿。
    <br> 
    <strong>9、</strong>用例包查查看分配历史，优化页面。
    <br> 
    <strong>10、</strong>公共用例库，产品用例库 ，脑图显示样式优化。
    <br> 
    <strong>11、</strong>进行优化，速度比之前好很多，项目少体现不出来，有用户 80多个项目后，改进明显。
    <br> 
    <strong>12、</strong>切换项目下的搜索框，过滤项目后，显示当前过滤条件，之前按回车后，清除当前过滤条件。
   </div> 
   <div>
    <strong>13、</strong>我的用例包视图，如下拉过滤中选择了某个测试包，执行用例，自动下一个时，也缺省是所选用例包
   </div>   
   <div> 
    <p><strong>1个bug修复:</strong></p> 
    <p><strong>1、</strong>接口用例包，特殊场景下，有时候参数引用不到。</p> 
