...
---
title: 'itest(爱测试) 接口测试，敏捷测试管理平台 10.1.4 发布'
categories: 
 - 编程
 - 开源中国
 - 资讯
...
author: 开源中国
comments: false
date: Fri, 20 Aug 2021 09:34:00 GMT
  ...
  <div> 
   <div> 
    <div> 
     ...
     <p><strong>v10.1.4  下载地址 </strong>：<a href="https://www.oschina.net/action/GoToLink?url=http%3A%2F%2F120.78.0.137%2Frsf%2Fsite%2Fitest%2Fdownload%2Findex.html" target="_blank">itest下载</a></p> 
     <p><strong>二：10.1.4 更新说明</strong></p> 
     <p>10.1.4  共<strong>23个更新含4个bug fix</strong>，如 jenkins build完发布版本后可以通过job 来执行itest上的接口测试，也支持gitLab 提交代码时，通过web hook 让itest 上的Bug 和gitLab 上的commit 关联等功能。<a href="https://www.oschina.net/action/GoToLink?url=http%3A%2F%2Fwww.itest.work%2Fitest%2Fversion%2FversionUpdate" target="_blank">查看版本更新历史</a>  <strong>户的持续反馈是我们不断更新的动力。</strong></p> 
     <div>
      <strong>19个更新：</strong>
     </div> 
     <div>
      1：增加安全key 设置，和gitLab 以及jenkins 打通时，回调itest 的接口时，使用配置的key 进行连接。
      <br> 2：build 完，部署后，可以在jenkins job 中调用itest 的接口，触发执行接口测，同时定时任务增加编号，方便作为jenkins 回调参数。
      <br> 3：接口测试定时任务执行完成后，自动把测试结果发邮件和钉钉 。
      <br> 4：bug导出的时候可以把bug标题另起一列吗。
      <br> 5：用例导出时，增加前置条件导出。
      <br> 6：用例导入进，增加前置条件导入。
      <br> 7:   用例同步线下处理时，处理前置条件。
      <br> 8：只要是测试人员都可能关闭别人提的BUG，之前只能提交人来关闭。
      <br> 9：开发人员可以处理其他开发人员的待改BUG 。
      <br> 10：接口测试定时任务执行完成后，发测试统计到邮件和钉钉。
      <br> 11：分配人员可以分配其他分配人员名下的 BUG到开发人员，之前只有分配本人名下的 ；仲裁人也作了类似处理。
      <br> 12：上版本用例增加了前置条件，新版，在用例库中，以及用例库和项目间双向同步用例时，也同步前置条件。
      <br> 13：执行用例时，只要更改了前置条件，在执行时，也会自动保存，这前只有更改了预期结果，描述及操作过程时才自动保存。
      <br> 14：ldap 配置以及同步用户进行优化。
      <br> 15：在测试用例包列表页面，增加执行用例入口，如是执行人，在这里也可以执行用例。
      <br> 16：之前操作提示信息的小窗口自关闭，更改这组件，可以自动关，也可以不关，比如同步线下用例，用例库和项目双向同步用例的提示就不用关。
      <br> 17：接口测试定时执行后台优化  。
      <br> 18：双向同步用例时，指定项目同步时，如选同步所有则只同步修改的记录，指定项目时，修改和新增都同步  。
      <br> 19：迭代导出时，用例导出增加一列  。
     </div> 
    </div> 
    <div> 
     <div> 
      <p><strong>4个bug修复:</strong></p> 
      <p>1：解决开发人员设置BUG 状态为重复，或是不计划修改 ，然后开发人员，再处理这BUG时，他可以直接关闭的BUG 。<br> 2：接口测试场景中，设置了测试环境，但是取不测试环境中配置的全局参数 。  <br> 3：接口测试场景中，设置了测试环境，执行测试的结果，没同步到接口用例的最新状态中，只更新了测试场景中的最新结果 。 <br> 4：修复双向同步用例的bug。</p> 
      <p><strong>三：10.X 新功能截图及9. X 版本之后新 UI </strong></p> 
