...
<p>过度滚动拉伸 - Google 还增加了一个新的系统范围内的 "拉伸" 滚动效果，让用户知道他们已经滚动到了用户界面中可用内容的末端。拉伸效果提供了一个自然的垂直和水平滚动停止的指示器，在所有的应用程序中都是通用的，它在整个平台和 AndroidX 的滚动容器中默认启用。新的拉伸滚动取代了以前版本中支持的 Glow 滚动。请确保用新的滚动行为测试你的应用程序和内容。</p> 
