...
---
title: 'Sqlbean1.5.6 发布，减少 Sql 语句，让你编码更高效'
categories: 
 - 编程
 - 开源中国
 - 资讯
...
<span><version>1.5.6</version></span>
...
<div style="text-align:left"> 
 <div> 
  <pre><span><span style="color:#888888">//标识表名</span></span>
<span><span style="color:#888888">//标识表名</span></span>
<span><span>@SqlTable</span><span>(</span><span style="color:#dd2200">"d_essay"</span><span>)</span></span>
...
<span>    <span>@SqlVersion</span></span>
...
<p>3.无需Dao层，Service层接口只需继承SqlBeanService<实体类, ID></p> 
<div style="text-align:left"> 
 <div> 
...
<p>4.Service实现类只需继承MybatisSqlBeanServiceImpl<实体类, ID>和实现你的Service接口</p> 
<div style="text-align:left"> 
 <div> 
...
<span>        <strong style="color:#445588">List</strong><span><</span><strong style="color:#445588">Essay</strong><span>></span> <span>list</span> <span>=</span> <span>essayService</span><span>.</span><span style="color:#008080">selectAll</span><span>();</span></span>
<span>        <span>list</span> <span>=</span> <span>essayService</span><span>.</span><span style="color:#008080">selectByCondition</span><span>(</span><strong style="color:#445588">Wrapper</strong><span>.</span><span style="color:#008080">where</span><span>(</span><span>gt</span><span>(</span><strong style="color:#445588">Essay</strong><span style="background-color:#ffadad; color:#a61717">$</span><span>.</span><span style="color:#008080">id</span><span>,</span> <span style="color:#009999">10</span><span>)).</span><span style="color:#008080">and</span><span>(</span><span>lt</span><span>(</span><strong style="color:#445588">Essay</strong><span style="background-color:#ffadad; color:#a61717">$</span><span>.</span><span style="color:#008080">id</span><span>,</span> <span style="color:#009999">20</span><span>)));</span></span>
...
<span>        <span>essay</span> <span>=</span> <span>essayService</span><span>.</span><span style="color:#008080">selectOneByCondition</span><span>(</span><strong style="color:#445588">Wrapper</strong><span>.</span><span style="color:#008080">where</span><span>(</span><span>eq</span><span>(</span><strong style="color:#445588">Essay</strong><span style="background-color:#ffadad; color:#a61717">$</span><span>.</span><span style="color:#008080">id</span><span>,</span> <span style="color:#009999">333</span><span>)));</span></span>
...
<span>        <span>select</span><span>.</span><span style="color:#008080">orderBy</span><span>(</span><strong style="color:#445588">Essay</strong><span style="background-color:#ffadad; color:#a61717">$</span><span>.</span><span style="color:#008080">id</span><span>,</span> <strong style="color:#445588">SqlSort</strong><span>.</span><span style="color:#008080">DESC</span><span>);</span></span>
...
<span>        <span style="color:#888888">//i = essayService.updateByCondition(Wrapper.where(gt(Essay$.id, 1)).and(eq(Essay$.content, "222")));</span></span>
...
<span>        <span style="color:#888888">//i = essayService.deleteByCondition(Wrapper.where(gt(Essay$.id, 1)).and(eq(Essay$.content, "222")));</span></span>
<span>        <strong style="color:#000000">if</strong> <span>(</span><span>i</span> <span>></span> <span style="color:#009999">0</span><span>)</span> <span>&#123;</span></span>
...
<p>本次更新内容：</p> 
<p><span style="background-color:#ffffff; color:#40485b">1：如果使用逻辑删除时Delete对象自动转Update对象进行逻辑删除；</span><br> <span style="background-color:#ffffff; color:#40485b">2：如果使用逻辑删除但该字段为null则自动填充；</span><br> <span style="background-color:#ffffff; color:#40485b">3：优化字段生成，非数据库字段过滤；</span><br> <span style="background-color:#ffffff; color:#40485b">4：重载排序方法；</span><br> <span style="background-color:#ffffff; color:#40485b">5：修复条件处理器；</span><br> <span style="background-color:#ffffff; color:#40485b">6：其他优化</span></p>
                                        </div>
