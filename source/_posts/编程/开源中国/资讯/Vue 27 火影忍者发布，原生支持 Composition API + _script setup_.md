...
<ul style="margin-left:0; margin-right:0"> 
 <li><code>createApp()</code><span>（</span>Vue 2 没有独立的 app scope）</li> 
 <li>Top-level<span> </span><code>await</code><span> </span>in<span> </span><code><script setup></code>（<span style="color:#374151">Vue 2 不支持异步组件初始化</span>）</li> 
 <li>在模板表达式中支持 TypeScript 语法（不兼容 w/ Vue 2 解析器）</li> 
 <li>响应式转换 Reactivity transform（仍处于实验性阶段）</li> 
 <li><code>expose</code><span> </span>选项不支持 options 组件（但<code><script setup></code>支持<code>defineExpose()</code> )</li> 
</ul> 
