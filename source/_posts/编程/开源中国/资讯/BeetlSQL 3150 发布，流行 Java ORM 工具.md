...
---
title: 'BeetlSQL 3.15.0 发布，流行 Java ORM 工具'
categories: 
 - 编程
 - 开源中国
 - 资讯
...
author: 开源中国
comments: false
date: Wed, 06 Jul 2022 00:12:00 GMT
thumbnail: 'https://picsum.photos/400/300?random=9276'
---

<div>   
<div class="content">
                                                                                            <p>本次发布主要是支持传统数据库存储过程调用，可以mapper方法使用或者直接使用SQLManager底层API</p> 
<pre><code class="language-java">@Call("call test.selectStu(?,?)")
List<OrderLog> callSample(@CallParm(1) int id, @CallOutBean  OutHolder outHolder);

@Call("call test.updateStu(?)")
@Update
int update(int id);</code></pre> 
<p>调用代码如下，</p> 
<pre><code class="language-java">OutHolder outHolder = new OutHolder();
...
<p>OutHolder类是用户定义的一个POJO，可以包含存储过程的出参</p> 
...
<pre style="margin-left:0; margin-right:0; text-align:left"><code class="language-xml"><span style="color:#333333"><<span style="color:#22863a">dependency</span>></span>
    ...
    <span style="color:#333333"><<span style="color:#22863a">version</span>></span>3.15.0-RELEASE<span style="color:#333333"></<span style="color:#22863a">version</span>></span>
