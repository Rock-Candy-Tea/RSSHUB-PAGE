...
---
title: 'RXThinkCMF 敏捷开发框架 ThinkPhp6.x+EleVue 版本 v1.6.0 发布'
categories: 
 - 编程
 - 开源中国
 - 资讯
...
<div>   
<div class="content">
                                                                                            <p>v1.6.0更新内容：<br> 1、更新优化城市管理模块；<br> 2、新增导出Excel功能，集成了两种案例，职级管理的后台导出以及登录日志的前端自动导出<br> 3、从国家地理信息往重新采集国家城市列表最新数据，涉及数据表"sys_city";<br> 4、菜单管理模块进行优化升级，列表采用异步加载机制，提高响应速度，编辑页新增权限节点的自动化维护穿梭组件；<br> 5、前端Vue端新增按钮权限节点控制，没有节点权限则按钮不显示，使用方式如：v-if="permission.includes('sys:level:delete')"<br> 6、完善个人中心基本资料的维护功能；<br> 7、职级管理、岗位管理、登录日志集成导出Excel功能；<br> 8、重新设计登录界面背景图片；<br> 9、解决切换账号之前打开的Tab页依然存在的问题；<br> 10、解决添加用户相同用户名系统未进行校验的问题，此版本已完善；</p> 
<h2>项目介绍</h2> 
