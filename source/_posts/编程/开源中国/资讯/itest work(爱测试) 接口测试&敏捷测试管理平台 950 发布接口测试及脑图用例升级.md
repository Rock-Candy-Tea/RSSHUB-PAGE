...
<div class="content">
 ...
 <p><strong>v9.5.0  beta  下载地址 </strong>：<a href="https://www.oschina.net/action/GoToLink?url=http%3A%2F%2F120.78.0.137%2Frsf%2Fsite%2Fitest%2Fdownload%2Findex.html" target="_blank">itest下载</a></p> 
 <p><strong>二：9.5.0 beta Release 说明</strong></p> 
 <p>9.0.5 <strong>GA </strong> 发布6天后，带着14个更新且含1个BUG修复的9.5.0  beta 又来了<strong>，用户的持续反馈是我们不断更新的动力!3月底发布新一版源码到开源中国上，当前忙着快速迭代，接口测试和实现压测功能。</strong></p> 
 <p><strong>9.5.0 beta 主要实现了 接口测试自定义采样器插件，自定义前置处理插件，自定议后置处理插件，都是通过上传自己的JAVA代码实现，可以是jar, 也可以是java 源文件; 以及用例脑图视图，上可以修改和执行标准用例，切换不同模块时，脑图显示为模块用例的脑图视图，切换到根就是整个项目的脑图视图。</strong></p> 
 <p>     截止9.0.1接口测试，已包含接口mock ,接口加密，解密和签名，接口参数化，接口间动态参数传寄，接口依赖推导，建测试测试场景时，自动加入依赖的接口并按依赖关系排好执行顺序，拖拽生成断言，拖拽提取参数  。<strong>另外在此也预告一下压测功能延期(计划赶不上变化)到4月中旬发布。</strong>下面先预览一下，9.0.X之后新UI</p> 
 ...
 <div>
  <strong><strong>9.5.0 beta详情如下:</strong></strong>
 </div> 
 <div>
  <strong>12个增强：</strong>
 </div> 
 <p><strong>1：</strong>工作空间加一个下拉，缺省是所有，下拉里有任务，测试包，迭代，缺陷，以方便按类型过滤 。<br> <strong>2：</strong>用例列表中，可以点用例状态可以执行用例，只是弹出的用例明细窗口中 有一个用例包的下接，点开这用例明细时，下拉列出该用例所属用例包 ，且是必选项，如该用例没属于任何用例包，不能执行 。<br> <strong>3:  </strong> 服图视图中右键也增加一个执行用例，方式同用例列表中执行用例 。<br> <strong>4: </strong>  修复当前执行用例窗口，因公共CSS被修复后，要拖溢滚动条，才能看到下面的操作的按钮的问题  。<br> <strong>5：</strong> 用例管理中再加一个我的用例包视图，左侧还是树 但右边是你的所有用例包下的用例，，用例列表上，可按用例包下拉查询， 同时用例列表中点用例状态可执行用例 。<br> <strong>6： </strong>用例脑图视图，左则显示树，点根就是整个项目用例脑图，点模块，脑图显示这摸块下的所有节点及用例 。<br> <strong>7：</strong>用例页面，对齐优化 。<br> <strong>8：</strong>用例脑图视图，可以增加标准用例以及，增加用例包，状态，版本三个下拉查询条件 。<br> <strong>9：</strong> 我的用例包视图，增加状态和优先级下拉查询条件<br> <strong>10：</strong>接口测试支持自定义采样器插件<br> <strong>11：</strong>接口测试支持自定义前置处理插件<br> <strong>12：</strong>接口测试支持自定义后置处理插件</p> 
 <p><strong>13：</strong> 增加致谢Contributors </p> 
 <div> 
  <p><strong>1个bug修复:</strong></p> 
  <p><strong>1：</strong>Bug 导出，点根节点导出BUG是空</p> 
