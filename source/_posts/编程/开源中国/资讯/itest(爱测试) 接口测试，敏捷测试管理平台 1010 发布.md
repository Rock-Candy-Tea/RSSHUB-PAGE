...
---
title: 'itest(爱测试) 接口测试，敏捷测试管理平台 10.1.0 发布'
categories: 
 - 编程
 - 开源中国
 - 资讯
...
author: 开源中国
comments: false
date: Tue, 03 Aug 2021 14:43:00 GMT
...
<div class="content">
 ...
 <div> 
  <div> 
   ...
   <p><strong>v10.1.0  下载地址 </strong>：<a href="https://www.oschina.net/action/GoToLink?url=http%3A%2F%2F120.78.0.137%2Frsf%2Fsite%2Fitest%2Fdownload%2Findex.html" target="_blank">itest下载</a></p> 
   <p><strong>二：10.1.0 更新说明</strong></p> 
   <p>10.1.0  <strong>16个更新，7个bug fix</strong>，<a href="https://www.oschina.net/action/GoToLink?url=http%3A%2F%2Fwww.itest.work%2Fitest%2Fversion%2FversionUpdate" target="_blank">查看版本更新历史</a>  <strong>户的持续反馈是我们不断更新的动力。</strong></p> 
   <div>
    <strong>9个增强：</strong>
   </div> 
   <div>
    1： 增加手工测试用例编排为业务场景功能。。
    <br> 2： 手工用例主用例编排为业务场景后，在用列不示显示，场景名 且查看详情显示也随之调整 。
    <br> 3： 手工用例执行时，如果业务场景用例，显示和执行方式以编排形式来执行 。
    <br> 4： 由接口用转生成JMX压测脚本时，自动转换接口断言，以及提取参数为jmx 形式，且把接口依赖关系也转到JMX中 ，实现接口用例无缝转JMX 。
    <br> 5： 手工用例包，执行页面，统计信息加入业务场景用例的统计 。
    <br> 6： 手工测试功能用例包，导入页面，优化。
    <br> 7： 用例页面优化，前置条件放出来显示了，然后过程及数据的框加高，滚动条滚动时按钮不动 。
    <br> 8： 删除压测节点时，如该节点被压测场景使用，不能删除 。
    <br> 9： 消息提示框显示2秒自动关闭，改为3秒后自动关闭 。
   </div> 
  </div> 
  <div> 
   <div> 
    <p><strong>7个bug修复:</strong></p> 
    <p>1： 接口测试，拖拽生成断言，如拖拽一个属性后 选择了操作符，但没没输入值，有时候再一次进入拖拽，页面不显示 。<br> 2： AD 域ldap 二因子认证时，因连接池化导致 ，用老密码也能登录 。<br> 3： 点压测试场景报 500 的错 。</p> 
    <p>4： 接口测试有多重依赖时，如出错，线程池没有shutdown。<br> 5： 接口测试响应数据，如不是json 有时候，不显示执行结果。<br> 6： 先进入项目的测试总结，再进项目维护，然后新增项目，结果发现进的是项目修改页面 。</p> 
    <p>7： 如接口参数为空行时，执行测试js 报错</p> 
    <p><strong>三：10.1.0 新功能截图及9. X 版本之后新 UI </strong></p> 
    ...
    <p>10.1.0 增加的手工功能测试用例编排</p> 
    <p style="text-align:left">       用例编排是为了重用用例，把单一的用例重组为业务场景用例。比如转帐，先要登录，查余额等操作，这些操作每一个都是一个单独的用例，把这几个用例按一定顺序组合一起，就是一个业务场景用例。虽然也可以在一个用例中，把上面的用例，作为一个步骤，但这带来的问题是用例不复用。</p> 
    <p> <img height="1043" src="https://oscimg.oschina.net/oscnet/up-672aea3092050eaf64864586636650365dd.png" width="1852" referrerpolicy="no-referrer"></p> 
    <p><img height="1005" src="https://oscimg.oschina.net/oscnet/up-b499d8e16a1117f65b712dddb5e971de995.png" width="1871" referrerpolicy="no-referrer"></p> 
